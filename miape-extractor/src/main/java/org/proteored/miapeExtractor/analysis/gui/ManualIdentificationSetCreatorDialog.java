...
import org.proteored.miapeExtractor.analysis.gui.tasks.IdentificationSetFromDTASelectFileTask;
 ...
 * @author __USER__
 */
public class ManualIdentificationSetCreatorDialog extends javax.swing.JDialog
	...
	private ManualIdentificationSetCreatorDialog(
			Miape2ExperimentListDialog parent) {
		super(parent, true);
		this.parent = parent;
		initComponents();
	...
	public static ManualIdentificationSetCreatorDialog getInstance(
			Miape2ExperimentListDialog parent) {
		if (instance == null) {
	...
	private void initComponents() {
		...
		jLabelInfo
				.setText("Here, you can create your own protein/peptide identification set.");

		jLabelInfo2
				.setText("You must use a tabular separated values (TSV) file with:");
		...
		jLabelInfo5
				.setText("An optional third column containing PSM score numbers.");

		jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(
				javax.swing.BorderFactory.createEtchedBorder(), "Input file"));
		...
		jButtonImport
				.setToolTipText("Save Identification Set and close dialog");
		...
		jPanel5.setBorder(javax.swing.BorderFactory.createTitledBorder(
				javax.swing.BorderFactory.createEtchedBorder(), "Name"));
		...
		layout.setVerticalGroup(layout
				...
				.addGroup(
						layout.createSequentialGroup()
								...
								.addPreferredGap(
										javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addComponent(jLabelInfo4)
								.addPreferredGap(
										javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addComponent(jLabelInfo5)
								.addGap(4, 4, 4)
	...
	private void importData() {
		...
		if (miapeMSIBuilder != null) {
			try {
				...
				final File manualIdSetFile = FileManager
						.getManualIdSetFile(name);
				if (manualIdSetFile != null && manualIdSetFile.exists()) {
					int selectedOption = JOptionPane
							.showConfirmDialog(
									this,
									"<html>There is already an identification set with the name '"
											+ name
											+ "'<br>"
											+ "It is located at: '"
											+ manualIdSetFile.getAbsolutePath()
											+ "'<br>"
											+ "Are you sure you want to override the data?",
									"Identification set overriding",
				...
				if (override) {
					final String saved = FileManager.saveManualIdSetMiapeMSI(
							name, miapeMSI);
					this.saved = true;
					parent.addReloadLocalIdentificationSetsTree();
					appendStatus("Data imported successfully. File created at: '"
							+ saved + "'");
				} else {
				...
				e.printStackTrace();
				appendStatus("Error saving identification set on "
						+ FileManager.getManualIdSetPath());
	...
	private void selectFile() {
		...
		if (selectedFile != null) {
			...
			final String name = FilenameUtils.getName(selectedFile
					.getAbsolutePath());
			if (name.contains(".")) {
			...
			identificationSetFromDTASelect = new IdentificationSetFromDTASelectFileTask(
					selectedFile, idSetName);
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if (IdentificationSetFromDTASelectFileTask.DTASELECT_PARSER_STARTS
		...
		} else if (IdentificationSetFromDTASelectFileTask.DTASELECT_PARSER_ERROR
				.equals(evt.getPropertyName())) {
			appendStatus("Error parsing file as DTASelect file: "
					+ evt.getNewValue());
			...
			String idSetName = jTextFieldName.getText();
			identificationSetFromFileParser = new IdentificationSetFromFileParserTask(
					selectedFile, idSetName, separator);
			identificationSetFromFileParser.addPropertyChangeListener(this);
			identificationSetFromFileParser.execute();

		} else if (IdentificationSetFromDTASelectFileTask.DTASELECT_PARSER_FINISHED
				.equals(evt.getPropertyName())) {
			...
			jLabelParsingResult.setText(numProteins + " proteins and "
			...
			jTextFieldFilePath.setEnabled(true);
			jTextFieldName.setEnabled(true);
		} else if (IdentificationSetFromFileParserTask.PARSER_STARTS.equals(evt
				.getPropertyName())) {
			...
			jTextFieldFilePath.setEnabled(false);
			jTextFieldName.setEnabled(false);
		} else if (IdentificationSetFromFileParserTask.PARSER_ERROR.equals(evt
				.getPropertyName())) {
			...
			jButtonClose.setEnabled(true);
			jButtonImport.setEnabled(false);
		} else if (IdentificationSetFromFileParserTask.PARSER_FINISHED
				.equals(evt.getPropertyName())) {
			log.info("MIAPE MSI created");
			saved = false;
			appendStatus("File parsed successfully.");
	...
	private int getNumProteins(MiapeMSIDocumentBuilder miapeMSIBuilder) {
		...
		if (miapeMSIBuilder != null) {
			...
			if (miapeMSI.getIdentifiedProteinSets() != null)
				for (IdentifiedProteinSet proteinSet : miapeMSI
