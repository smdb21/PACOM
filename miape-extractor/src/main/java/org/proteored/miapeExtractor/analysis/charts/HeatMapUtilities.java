...
public abstract class HeatMapUtilities {
	 ...
	 *            the row (y) index.
	 * @param seriesName
	 *            the series name/key (<code>null</code> not permitted).
	 * 
	 * @return The dataset.
	 */
	public static XYDataset extractRowFromHeatMapDataset(HeatMapDataset dataset, int row,
	 ...
	 *            the dataset (<code>null</code> not permitted).
	 * @param paintScale
	 *            the paint scale for the z-values (<code>null</code> not
	 *            permitted).
	 * 
	...
	public static BufferedImage createHeatMapImage(HeatMapDataset dataset, PaintScale paintScale,
		...
		BufferedImage image = new BufferedImage(xCount * cellWidth, yCount * cellHeight,
		...
		for (int xIndex = 0; xIndex < xCount; xIndex++) {
			for (int yIndex = 0; yIndex < yCount; yIndex++) {
				...
				g2.setPaint(p);
				g2.fillRect(xIndex, yCount - yIndex - 1, cellWidth - 1, cellHeight - 1);
			}
		}
		return image;
	}
