...
import org.proteored.miapeExtractor.analysis.conf.ExperimentAdapter;
...
import org.proteored.miapeExtractor.analysis.gui.components.MyTreeRenderer;
import org.proteored.miapeExtractor.analysis.gui.tasks.ExternalDataTreeLoaderTask;
import org.proteored.miapeExtractor.analysis.gui.tasks.InitializeProjectComboBoxTask;
import org.proteored.miapeExtractor.analysis.gui.tasks.LocalDataTreeLoaderTask;
import org.proteored.miapeExtractor.analysis.gui.tasks.MiapeRetrieverManager;
import org.proteored.miapeExtractor.analysis.gui.tasks.MiapeRetrieverTask;
import org.proteored.miapeExtractor.analysis.gui.tasks.MiapeTreeIntegrityCheckerTask;
...
import org.proteored.miapeExtractor.gui.tasks.TreeLoaderTask;
 ...
 * @author __USER__
 */
public class Miape2ExperimentListDialog extends javax.swing.JFrame implements
		PropertyChangeListener {
	public static final String MIAPE_CONVERTER_PROPERTIES_FILE = "miape-extractor.properties";
	...
	final static String MIAPE_ID_REGEXP = "MIAPE\\s\\w+\\s(\\d+).*";
	final static String LOCAL_MIAPE_ID_REGEXP = "LOCAL_MIAPE_\\w+_(\\d+).*";
	final static String MIAPE_NAME_REGEXP = "(MIAPE\\s\\w+\\s\\d+).*";
	final static String LOCAL_MIAPE_NAME_REGEXP = "(LOCAL_MIAPE_\\w+_\\d+).*";
	...
	public void dispose() {
		...
		if (treeLoaderTask != null
				&& treeLoaderTask.getState() == StateValue.STARTED) {
		...
		if (externalDataTreeLoaderTask != null
				&& externalDataTreeLoaderTask.getState() == StateValue.STARTED) {
		...
		if (projectComboLoaderTask != null
				&& projectComboLoaderTask.getState() == StateValue.STARTED) {
	...
	private Miape2ExperimentListDialog(MainFrame parent) {
		...
		try {
			if (MainFrame.userName != null && !"".equals(MainFrame.userName)
					&& MainFrame.password != null
				...
				appendStatus("MIAPE MSI documents cannot be loaded since the user has not logged-in.");
				appendStatus("Load a previously saved comparison project or login again.");
			}
		...
		MiapeRetrieverManager.getInstance(MainFrame.userName,
				MainFrame.password).setListener(this);
	...
	private void fillTreeFromProteoRedRepository() {
		if (MainFrame.userName != null && !"".equals(MainFrame.userName)
				&& MainFrame.password != null && !"".equals(MainFrame.password)) {
			if (treeLoaderTask != null)
				treeLoaderTask.cancel(true);
			treeLoaderTask = new TreeLoaderTask(null, jTreeMIAPEMSIs, null,
					null, MainFrame.userName, MainFrame.password);
	...
	private void fillExternalIdSetsTree() {
		externalDataTreeLoaderTask = new ExternalDataTreeLoaderTask(
				jTreeManualMIAPEMSIs);
	...
	private void fillLocalMIAPETree() {
		localDataTreeLoaderTask = new LocalDataTreeLoaderTask(
				jTreeLocalMIAPEMSIs);
	...
	private void loadIcons() {
		...
		jButtonStartLoading.setIcon(ImageManager
				.getImageIcon(ImageManager.RELOAD));
		jButtonStartLoading.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.RELOAD_CLICKED));
		jButtonCancelLoading.setIcon(ImageManager
				.getImageIcon(ImageManager.STOP));
		jButtonCancelLoading.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.STOP_CLICKED));
		jButtonSave.setIcon(ImageManager.getImageIcon(ImageManager.SAVE));
		jButtonSave.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.SAVE_CLICKED));
		jButtonFinish.setIcon(ImageManager.getImageIcon(ImageManager.FINISH));
		jButtonFinish.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.FINISH_CLICKED));
		jButtonLoadSavedProject.setIcon(ImageManager
				.getImageIcon(ImageManager.LOAD));
		jButtonLoadSavedProject.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.LOAD_CLICKED));

		jButtonRemoveSavedProject.setIcon(ImageManager
				.getImageIcon(ImageManager.DELETE));
		jButtonRemoveSavedProject.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.DELETE_CLICKED));

		jButtonAddCuratedExperiment.setIcon(ImageManager
				.getImageIcon(ImageManager.ADD));
		jButtonAddCuratedExperiment.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.ADD_CLICKED));
		jButtonAddExperiment.setIcon(ImageManager
				.getImageIcon(ImageManager.ADD));
		jButtonAddExperiment.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.ADD_CLICKED));
		jButtonAddReplicate
				.setIcon(ImageManager.getImageIcon(ImageManager.ADD));
		jButtonAddReplicate.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.ADD_CLICKED));
		jButtonClearProjectTree.setIcon(ImageManager
				.getImageIcon(ImageManager.CLEAR));
		jButtonClearProjectTree.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.CLEAR_CLICKED));
		jButtonDeleteNode.setIcon(ImageManager
				.getImageIcon(ImageManager.DELETE));
		jButtonDeleteNode.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.DELETE_CLICKED));
		jButtonRemoveCuratedExperiment.setIcon(ImageManager
				.getImageIcon(ImageManager.DELETE));
		jButtonRemoveCuratedExperiment.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.DELETE_CLICKED));
	...
	private void initComponents() {
		...
		setTitle("MIAPE data inspection project manager");

		jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(
				javax.swing.BorderFactory.createEtchedBorder(), "MIAPE MSI"));

		jTabbedPane.setToolTipText("MIAPE documents stored in local system");

		jPanel10.setToolTipText("MIAPE documents in remote repository");
		...
		jPanel12.setToolTipText("MIAPE documents stored in local system");
		...
		jTabbedPane.addTab("Local data", jPanel12);

		jPanel11.setToolTipText("External identification data");
		...
		jLabel8.setText("Add new identification lists:");

		jButtonAddManualList.setText("Add");
		jButtonAddManualList
		...
		jTabbedPane.addTab("External data", jPanel11);
		...
		jPanel1Layout.setHorizontalGroup(jPanel1Layout.createParallelGroup(
				...
				jTabbedPane, javax.swing.GroupLayout.DEFAULT_SIZE, 292,
				Short.MAX_VALUE));
		...
		jPanel2Layout.setHorizontalGroup(jPanel2Layout.createParallelGroup(
				...
				jPanel2Layout
						...
						.addComponent(jScrollPane2,
								javax.swing.GroupLayout.DEFAULT_SIZE, 252,
								Short.MAX_VALUE).addContainerGap()));
		...
		jPanel6.setBorder(javax.swing.BorderFactory.createTitledBorder(
				javax.swing.BorderFactory.createEtchedBorder(),
				"Loaded MIAPE MSI documents"));

		jLabel5.setText("MIAPE MSI documents:");

		jLabelMIAPEMSINumber.setText("0");

		jButtonCancelLoading.setText("Stop loading MIAPE tree");
		jButtonCancelLoading.setEnabled(false);
		jButtonCancelLoading
				.addActionListener(new java.awt.event.ActionListener() {
					@Override
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		jButtonStartLoading.setText("Reload MIAPE tree");
		jButtonStartLoading
				.addActionListener(new java.awt.event.ActionListener() {
					@Override
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		jPanel3.setBorder(javax.swing.BorderFactory.createTitledBorder(
				javax.swing.BorderFactory.createEtchedBorder(), "Edit"));
		...
		jLabel2.setText("Level 1 Name (experiment):");
		...
		jTextExperimentName
				.setToolTipText("<html> The level 1 can represent an experiment or simply a <br> compilation of peptide and protein identification sets. </html>");
		...
		jButtonAddExperiment
				.setToolTipText("Click here to add a new level 1 node to the inspection tree.");
		jButtonAddExperiment
				.addActionListener(new java.awt.event.ActionListener() {
					@Override
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		jLabel3.setText("Level 2 Name (replicate/fraction/band):");
		...
		jTextReplicateName
				.setToolTipText("<html> The level 2 can represent a peptide and protein identification set comming from:<br> <ul> <li>a technical/biological replicate,</li> <li>a fraction in a LC separation,</li> <li>a gel spot,</li> <li>or a gel band.</li> </html>");
		...
		jButtonAddReplicate
				.setToolTipText("Click here to add a new level 2 node to the inspection tree.");
		jButtonAddReplicate
				.addActionListener(new java.awt.event.ActionListener() {
					@Override
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		jPanel8.setToolTipText("<html>\nThe label generator template defines which names will be<br>\nautomatically assigned to the Level 2 nodes<br> (replicates/fractions/bands)</html>");

		jTextFieldLabelNameTemplate.setText("replicate:");
		jTextFieldLabelNameTemplate
				.setToolTipText("<html>The prefix of the name of the 'identification set'<br>\nwhen adding MIAPE MSIs to the comparison project.</html>");

		jLabel4.setText("Name:");
		jLabel4.setToolTipText("<html>The prefix of the name of the 'identification set'<br> when adding MIAPE MSIs to the comparison project.</html>");

		jLabel6.setText("Number:");
		jLabel6.setToolTipText("<html>The sufix of the name of the 'identification set'<br>\nwhen adding MIAPE MSIs to the comparison project.<br>\nIf this text is a number, it will be increment in sucessive addings.</html>");

		jTextFieldLabelNumberTemplate.setText("1");
		jTextFieldLabelNumberTemplate
				.setToolTipText("<html>The sufix of the name of the 'identification set'<br> when adding MIAPE MSIs to the comparison project.<br> If this text is a number, it will be increment in sucessive addings.</html>");
		...
		jPanel9.setToolTipText("<html>\n<b>Curated experiments<b/> are created in the MIAPE Extractor Charts<br> viewer, usually after appling some filters.<br> This curated projects are lighter than normal projects<br> since filtered-out data is discarted and is not loaded.</html>");

		jComboBoxCuratedExperiments
				.setModel(new javax.swing.DefaultComboBoxModel(
						new String[] { "No curated experiments available" }));
		jComboBoxCuratedExperiments
		...
		jButtonRemoveCuratedExperiment
		...
		jButtonFinish
				.setToolTipText("Click to go to the MIAPE Extractor Charts Viewer");
	...
	private void localMiapeTreeClicked(MouseEvent evt) {
		...
		if (evt.getClickCount() == 2) {
			if (jTreeLocalMIAPEMSIs.isOnlyOneNodeSelected(MIAPE_LEVEL)) {
				// get MIAPE ID
				String miapeID = jTreeLocalMIAPEMSIs
						.getStringFromSelection(LOCAL_MIAPE_ID_REGEXP);
				...
				Integer ms_id = getAssociatedLocalMIAPEMSId(msi_id, projectName);
				// if a replicate is selected
				if (jTreeProject.isOnlyOneNodeSelected(REPLICATE_LEVEL)) {

					DefaultMutableTreeNode replicateNode = jTreeProject
							.getSelectedNode();
					CPReplicate cpReplicate = (CPReplicate) replicateNode
							.getUserObject();
					// String miapeNodeName = getMIAPENodeNameFromTemplate();

					String miapeNodeName = FileManager
							.getMiapeMSILocalFileName(Integer.valueOf(miapeID));
					...
					if (ms_id != null) {
						...
						CPMS cpMS = new CPMS();
						cpMS.setId(ms_id);
						cpMS.setName(FileManager.getMiapeMSLocalFileName(ms_id));
					...
					DefaultMutableTreeNode experimentNode = jTreeProject
							.getSelectedNode();
					CPExperiment cpExp = (CPExperiment) experimentNode
							.getUserObject();
					if (cpExp.isCurated())
					...
					final DefaultMutableTreeNode replicateNode = jTreeProject
							.addNewNode(cpRep, experimentNode);
					// add miape node
					CPMSI cpMsi = new CPMSI();
					cpMsi.setId(msi_id);
					cpMsi.setLocal(true);
					...
					cpMsi.setName(FileManager.getMiapeMSILocalFileName(Integer
							.valueOf(miapeID)));
			...
			} else if (jTreeLocalMIAPEMSIs
					.isOnlyOneNodeSelected(MIAPE_PROJECT_LEVEL)) {
				// if the user clicks on a MIAPE PROJECT
				log.info("Miape local project selected");
				final int numMIAPEsInMIAPEProject = jTreeLocalMIAPEMSIs
						.getSelectedNode().getChildCount();
				if (numMIAPEsInMIAPEProject > 0) {
					...
					final int selectedOption = JOptionPane
							.showConfirmDialog(
									...
									"<html>Do you want to add all MIAPEs in the MIAPE project to a new level 1 node called '"
											+ localProjectName
											+ "' in the inspection project?</html>",
									"Add MIAPEs to inspection project",
									JOptionPane.YES_NO_CANCEL_OPTION);
					if (selectedOption == JOptionPane.YES_OPTION) {
						DefaultMutableTreeNode projectNode = jTreeProject
								.getRootNode();
						CPExperimentList cpExpList = (CPExperimentList) projectNode
								.getUserObject();
						CPExperiment cpExp = new CPExperiment();
						cpExp.setName(localProjectName);
						cpExp.setCurated(false);
						// Add a new experiment node
						final DefaultMutableTreeNode experimentNode = jTreeProject
								.addNewNode(cpExp, projectNode);

						jTextExperimentName.setText(localProjectName);

						final Enumeration children = jTreeLocalMIAPEMSIs
								.getSelectedNode().children();
						while (children.hasMoreElements()) {
							DefaultMutableTreeNode miapeMSIChild = (DefaultMutableTreeNode) children
									.nextElement();
							String msi_id = ExtendedJTree.getString(
									LOCAL_MIAPE_ID_REGEXP,
									(String) miapeMSIChild.getUserObject());
							Integer ms_id = getAssociatedLocalMIAPEMSId(
									Integer.valueOf(msi_id), localProjectName);
							// start retrieving
					...
					throw new IllegalMiapeArgumentException(
							"The MIAPE Project has not MIAPE MSIs!");
				}
			}
		}
	...
	private Integer getAssociatedLocalMIAPEMSId(Integer id, String projectName) {
		final File msiFile = new File(
				FileManager.getMiapeLocalDataPath(projectName)
						+ FileManager.getMiapeMSILocalFileName(id));
		if (msiFile.exists()) {
			MiapeHeader miapeMSIHeader;
			try {
				final MIAPEMSIXmlFile miapemsiXmlFile = new MIAPEMSIXmlFile(
						msiFile);
				miapemsiXmlFile.setCvUtil(OntologyLoaderTask.getCvManager());
				MiapeMSIDocument miapeDocument = miapemsiXmlFile.toDocument();
	...
	private void jButtonRemoveSavedProjectMouseClicked(
	...
	private void manualMiapeTreeClicked(MouseEvent evt) {
		...
		if (evt.getClickCount() == 2) {
			if (jTreeManualMIAPEMSIs.isOnlyOneNodeSelected(EXPERIMENT_LEVEL)) {
				// get MIAPE ID
				String miapeName = jTreeManualMIAPEMSIs
	...
	private void jButtonAddManualListActionPerformed(
	...
	private void addManualIdSet() {
		...
		ManualIdentificationSetCreatorDialog dialog = ManualIdentificationSetCreatorDialog
				.getInstance(this);
		dialog.setVisible(true);
	}

	private void jComboBoxCuratedExperimentsActionPerformed(
			java.awt.event.ActionEvent evt) {

		final Object object = jComboBoxCuratedExperiments.getSelectedItem();
		if (NO_CURATED_EXPERIMENTS_AVAILABLE.equals(object)
				|| "".equals(object)) {
			jButtonAddCuratedExperiment.setEnabled(false);
			jButtonRemoveCuratedExperiment.setEnabled(false);
		} else {
	...
	private void jButtonRemoveCuratedExperimentActionPerformed(
			java.awt.event.ActionEvent evt) {
		final Object selectedItem = jComboBoxCuratedExperiments
				.getSelectedItem();
	...
	private void removeCuratedExperiment(String curatedExperimentName) {
		int selectedOption = JOptionPane
				.showConfirmDialog(
						this,
						"<html>Curated experiment '"
								+ curatedExperimentName
								+ "' will be removed from the disk.<br>Do you really want to continue?</html>",
						"Add MIAPEs to inspection project",
						JOptionPane.YES_NO_CANCEL_OPTION);
		if (selectedOption == JOptionPane.YES_OPTION) {
			log.info("Removing curated experiment files ... "
					+ curatedExperimentName);
			jButtonRemoveCuratedExperiment.setEnabled(false);
			boolean removed = FileManager
					.removeCuratedExperimentFiles(curatedExperimentName);
			if (removed)
				appendStatus("Curated experiment '" + curatedExperimentName
						+ "' removed succesfully");
			else
				appendStatus("Some error occurred while removing curated experiment '"
						+ curatedExperimentName + "'");
	...
	private void jButtonAddCuratedExperimentActionPerformed(
			java.awt.event.ActionEvent evt) {
		// See if the project list node is selected

		final String selectedItem = (String) jComboBoxCuratedExperiments
				.getSelectedItem();
		if (selectedItem != null
				&& !NO_CURATED_EXPERIMENTS_AVAILABLE.equals(selectedItem)
				&& !"".equals(selectedItem)) {
			final String curatedExperimentXMLFilePath = FileManager
					.getCuratedExperimentXMLFilePath(selectedItem);
			...
			if (curatedExpFile.exists()) {
				...
				appendStatus("Error loading curated experiment: "
						+ selectedItem + "at: "
	...
	private void jButtonStartLoadingActionPerformed(
			java.awt.event.ActionEvent evt) {
		if (treeLoaderTask != null
				&& treeLoaderTask.getState().equals(StateValue.DONE)) {
			treeLoaderTask.cancel(true);
	...
	private void jTextProjectNameKeyReleased(java.awt.event.KeyEvent evt) {
		...
		if (!"".equals(projectName)) {
			...
			final DefaultTreeModel model = (DefaultTreeModel) jTreeProject
					.getModel();
			if (model == null) {
				initializeProjectTree(DEFAULT_PROJECT_NAME,
						DEFAULT_EXPERIMENT_NAME);
			} else {
				DefaultMutableTreeNode root = (DefaultMutableTreeNode) model
						.getRoot();
				CPExperimentList cpExpList = (CPExperimentList) root
						.getUserObject();
	...
	private void jButtonCancelLoadingActionPerformed(
			java.awt.event.ActionEvent evt) {
		if (treeLoaderTask != null)
			treeLoaderTask.cancel(true);
	...
	private void jButtonDeleteNodeActionPerformed(java.awt.event.ActionEvent evt) {
		if (jTreeProject.getSelectionCount() > 0) {
			if (jTreeProject.isOnlyOneNodeSelected(PROJECT_LEVEL)) {
				...
				final CPExperiment cpExp = (CPExperiment) jTreeProject
						.getSelectedNode().getUserObject();
				final CPExperimentList cpExpList = (CPExperimentList) jTreeProject
						.getRootNode().getUserObject();
				cpExpList.getCPExperiment().remove(cpExp);

			} else if (jTreeProject.isOnlyOneNodeSelected(REPLICATE_LEVEL)) {
				final CPReplicate cpRep = (CPReplicate) jTreeProject
						.getSelectedNode().getUserObject();
				CPExperiment cpExp = (CPExperiment) ((DefaultMutableTreeNode) jTreeProject
						.getSelectedNode().getParent()).getUserObject();
				cpExp.getCPReplicate().remove(cpRep);

			} else if (jTreeProject
					.isOnlyOneNodeSelected(MIAPE_REPLICATE_LEVEL)) {
				final CPMSI cpMSI = (CPMSI) jTreeProject.getSelectedNode()
						.getUserObject();
				CPReplicate cpRep = (CPReplicate) ((DefaultMutableTreeNode) jTreeProject
						.getSelectedNode().getParent()).getUserObject();
	...
	private void jButtonClearProjectTreeActionPerformed(
			java.awt.event.ActionEvent evt) {
		initializeProjectTree(DEFAULT_PROJECT_NAME, DEFAULT_EXPERIMENT_NAME);
	...
	private void jButtonLoadSavedProjectMouseClicked(
			java.awt.event.MouseEvent evt) {
		String projectName = (String) jComboBox1.getSelectedItem();
		if (projectName != null && !"".equals(projectName)) {

			String projectXMLFilePath = FileManager
					.getProjectXMLFilePath(projectName);
			File projectFile = new File(projectXMLFilePath);
			if (!projectFile.exists()) {

				appendStatus("Project '" + projectName
						+ "' has not found in the projects folder");
				return;
		...
		if (evt.getClickCount() == 2) {
			if (jTreeMIAPEMSIs.isOnlyOneNodeSelected(MIAPE_LEVEL)) {
				...
				if (jTreeProject.isOnlyOneNodeSelected(REPLICATE_LEVEL)) {
					...
					String miapeNodeName = FileManager
							.getMiapeMSIFileName(Integer.valueOf(miapeID));
					...
					cpMsi.setName(FileManager.getMiapeMSIFileName(Integer
							.valueOf(miapeID)));
					CPMSIList cpMsiList = new CPMSIList();
					cpMsiList.getCPMSI().add(cpMsi);
					cpRep.setCPMSIList(cpMsiList);
			...
			} else if (jTreeMIAPEMSIs
					.isOnlyOneNodeSelected(MIAPE_PROJECT_LEVEL)) {
				// if the user clicks on a MIAPE PROJECT
				log.info("Miape project selected");
				final int numMIAPEsInMIAPEProject = jTreeMIAPEMSIs
						.getSelectedNode().getChildCount();
				if (numMIAPEsInMIAPEProject > 0) {

					// IF the node 0 is selected in the inspection project
					String experimentName = jTreeMIAPEMSIs
							.getStringFromSelection(MIAPE_PROJECT_NAME_REGEXP);
					final int selectedOption = JOptionPane
							.showConfirmDialog(
									this,
									"<html>Do you want to add all MIAPEs in the MIAPE project to a new level 1 node called '"
					...
					if (selectedOption == JOptionPane.YES_OPTION) {
						...
						final Enumeration children = jTreeMIAPEMSIs
								.getSelectedNode().children();
						while (children.hasMoreElements()) {
							DefaultMutableTreeNode miapeMSIChild = (DefaultMutableTreeNode) children
									.nextElement();
							String miapeID = ExtendedJTree.getString(
									MIAPE_ID_REGEXP,
									(String) miapeMSIChild.getUserObject());
							// start retrieving
							startMIAPEMSIRetrieving(Integer.valueOf(miapeID));
	...
	private String getMIAPENodeNameFromTemplate() {
		...
		if (!"".equals(prefix) || !"".equals(sufix)) {
			...
			miapeNodeName = (String) jTreeMIAPEMSIs.getSelectedNode()
					.getUserObject();
		}
	...
	private void incrementSuffixIfNumber() {
		try {
			Integer num = Integer.valueOf(jTextFieldLabelNumberTemplate
					.getText());
			num++;
	...
	private boolean save(boolean checkTree)
		...
		if (checkTree) {
			int selectedOption = JOptionPane
					.showConfirmDialog(
							this,
							"<html>Before to save the comparison project, some aspect should be checked:<br>"
									+ "<ul><li>In order to apply an FDR threshold, the MIAPE MSIs belonging to the same<br>"
									+ "'level 1' node must have been searched by the same search engine.</li>"
									+ "<li>The same name for different level 1 nodes is not allowed.</li>"
									+ "<li>The same name for different level 2 nodes belonging to the same level 1 node is not allowed.</ul><br>"
									+ "Do you want to check the integrity of your project now?</html>",
							"Check integrity of the project?",
							JOptionPane.YES_NO_OPTION);
			if (selectedOption == JOptionPane.YES_OPTION) {
				MiapeTreeIntegrityCheckerTask integrityChecker = new MiapeTreeIntegrityCheckerTask(
						this, cpExpList);
		...
		final String projectXMLFilePath = FileManager
				.getProjectXMLFilePath(cpExpList.getName());
		if (FileManager.existsProjectXMLFile(cpExpList.getName())) {
			// Show dialog
			final int option = JOptionPane
					.showConfirmDialog(
							this,
							"<html>The protein/peptide identification comparison project '"
									+ cpExpList.getName()
									+ "' already exists.<br>Are you sure you want to overwrite it?</html>",
							"Warning, project file already exists",
							JOptionPane.YES_NO_OPTION);
	...
	private CPExperimentList getCPExperimentListFromTree() {
		...
		if (userObject instanceof CPExperimentList) {
			...
			final List<CPExperiment> cpExps = cpExperimentList
					.getCPExperiment();
			if (cpExps != null)
				for (CPExperiment cpExp : cpExps) {
					...
					if (cpReps != null)
						for (CPReplicate cpRep : cpReps) {
							...
							if (cpMsList == null || cpMsList.getCPMS() == null
									|| cpMsList.getCPMS().isEmpty()) {
								final CPMSIList cpmsiList = cpRep
										.getCPMSIList();
								cpMsList = new CPMSList();
								if (cpmsiList != null) {
									for (CPMSI cpMSI : cpmsiList.getCPMSI()) {
										if (((cpMSI.isLocal() != null && !cpMSI
												.isLocal()) || cpMSI.isLocal() == null)
												&& MiapeRetrieverManager
														.getInstance(
																MainFrame.userName,
																MainFrame.password)
														.getMiapeAssociations()
														.containsKey(
																cpMSI.getId())) {
											CPMS cpMS = new CPMS();
	...
	private void jTextReplicateNameKeyReleased(java.awt.event.KeyEvent evt) {
		...
		if (jTreeProject.isOnlyOneNodeSelected(REPLICATE_LEVEL)) {
			final DefaultMutableTreeNode selectedNode = jTreeProject
					.getSelectedNode();
			...
			try {
				...
				if (cpExp.isCurated()) {
					if (!previousExpName.equals(newExpName)) {
						// RENAME THE FOLDER
						File curatedExpFolder = new File(
								FileManager
										.getCuratedExperimentFolderPath(previousExpName));
						File newCuratedExpFolder = new File(
								FileManager
										.getCuratedExperimentFolderPath(newExpName));
						curatedExpFolder.renameTo(newCuratedExpFolder);

						// RENAME THE CURATED EXPERIMENT FILE
						File curatedExpFile = new File(
								FileManager.getCuratedExperimentsDataPath()
										+ FileManager.PATH_SEPARATOR
										+ newExpName
										+ FileManager.PATH_SEPARATOR
										+ previousExpName + ".xml");
						File newCuratedExpFile = new File(
								FileManager
										.getCuratedExperimentXMLFilePath(newExpName));
						curatedExpFile.renameTo(newCuratedExpFile);
						CPExperiment cpExp2 = FileManager
								.getCPExperiment(newCuratedExpFile);
						if (cpExp2 != null) {
	...
	private void jButtonAddReplicateActionPerformed(
			java.awt.event.ActionEvent evt) {
		try {
			addReplicate();
		} catch (IllegalMiapeArgumentException e) {
	...
	private void addReplicate() {
		...
		if (!"".equals(replicateName)) {
			...
			if (model != null) {
				if (jTreeProject.isOnlyOneNodeSelected(EXPERIMENT_LEVEL)) {
					if (jTreeMIAPEMSIs.isOnlyOneNodeSelected(MIAPE_LEVEL)) {
						int miapeID = Integer.valueOf(jTreeMIAPEMSIs
								.getStringFromSelection(MIAPE_ID_REGEXP));

						startMIAPEMSIRetrieving(miapeID);

						// get selected experiment node
						DefaultMutableTreeNode experimentTreeNode = jTreeProject
								.getSelectedNode();
						CPExperiment cpExp = (CPExperiment) experimentTreeNode
								.getUserObject();
						if (cpExp.isCurated())
							throw new IllegalMiapeArgumentException(
									"A non curated MIAPE MSI cannot be added to a curated experiment");
						...
						final DefaultMutableTreeNode replicateTreeNode = jTreeProject
								.addNewNode(cpRep, experimentTreeNode);
						String miapeName = (String) jTreeMIAPEMSIs
								.getSelectedNode().getUserObject();
						CPMSI cpMSI = new CPMSI();
						cpMSI.setName(miapeName);
						cpMSI.setId(miapeID);
					...
					// get selected experiment node
					DefaultMutableTreeNode replicateTreeNode = jTreeProject
							.getSelectedNode();
					CPReplicate cpRep = (CPReplicate) replicateTreeNode
							.getUserObject();

					// add the miape node on the replicate node
					String miapeNodeName = (String) jTreeMIAPEMSIs
							.getSelectedNode().getUserObject();
					CPMSI cpMSI = new CPMSI();
					cpMSI.setName(miapeNodeName);
					cpMSI.setId(miapeID);
					...
					throw new IllegalMiapeArgumentException(
							"Select a level 1 node before to add a level 2 node");
				}
			}
		} else {
			throw new IllegalMiapeArgumentException(
					"Type a name for the level 2 node before to click on Add button");
		}
	...
	private void startMIAPEMSIRetrieving(int miapeID) {
		...
		if (file.exists()) {
			...
			if (MiapeRetrieverManager.getInstance(MainFrame.userName,
					MainFrame.password).getSize() == 0) {
				jProgressBar.setIndeterminate(false);
				jProgressBar.setStringPainted(false);
			}
			...
			MiapeRetrieverManager.getInstance(MainFrame.userName,
					MainFrame.password).addRetrieving(miapeID, "MSI", this);
			return;
		}
		...
		final String executingTasks = MiapeRetrieverManager.getInstance(
				MainFrame.userName, MainFrame.password)
				.addRetrievingWithPriority(miapeID, "MSI", this);

		appendStatus("Downloading MIAPE MSI " + executingTasks
				+ " from ProteoRed repository.");
		appendStatus("This task will be performed in backgroud.");

	}

	private void jButtonAddExperimentActionPerformed(
			java.awt.event.ActionEvent evt) {
		try {
			addExperiment();
		} catch (IllegalMiapeArgumentException e) {
		...
		if (!"".equals(experimentName)) {
			...
			if (model != null) {
				DefaultMutableTreeNode parentNode = (DefaultMutableTreeNode) model
						.getRoot();
				final CPExperimentList cpExpList = (CPExperimentList) parentNode
						.getUserObject();
				cpExpList.getCPExperiment().add(cpExp);
				final DefaultMutableTreeNode newNode = jTreeProject.addNewNode(
						cpExp, parentNode);
			...
			throw new IllegalMiapeArgumentException(
					"Type a name for the level 1 node before to click on Add button");
		}
	...
	private void initializeProjectTree(String projectName, String experimentName) {
		...
		cpExpList.setName(projectName);
		DefaultMutableTreeNode nodoRaizMSI = new DefaultMutableTreeNode(
				cpExpList);
		...
		cpExp.setName(experimentName);
		cpExpList.getCPExperiment().add(cpExp);
		final DefaultMutableTreeNode experimentNode = jTreeProject.addNewNode(
				cpExp, nodoRaizMSI);
	...
	private void initializeProjectTree(File projectFile) {
		CPExperimentList cpExperimentList = new ExperimentListAdapter(
				projectFile).getCpExperimentList();
		...
		jTreeProject.setModel(modeloArbolMSI);
		jTextProjectName.setText(projectName);
		final Iterator<CPExperiment> cpExpIterator = cpExperimentList
				.getCPExperiment().iterator();
		while (cpExpIterator.hasNext()) {
			CPExperiment cpExperiment = cpExpIterator.next();
			boolean curated = cpExperiment.isCurated();
			final DefaultMutableTreeNode experimentNode = jTreeProject
					.addNewNode(cpExperiment, nodoRaizMSI);
			boolean hasOneReplicate = false;
			final Iterator<CPReplicate> cpRepIterator = cpExperiment
					.getCPReplicate().iterator();
			while (cpRepIterator.hasNext()) {
				CPReplicate cpReplicate = cpRepIterator.next();
				final DefaultMutableTreeNode replicateNode = jTreeProject
						.addNewNode(cpReplicate, experimentNode);

				if (cpReplicate.getCPMSIList() != null) {
					for (CPMSI cpMsi : cpReplicate.getCPMSIList().getCPMSI()) {
						if (cpMsi.isManuallyCreated() == null
								|| !cpMsi.isManuallyCreated()) {
							if (cpMsi.isLocal() == null || !cpMsi.isLocal()) {
								boolean addNode = true;
								final Integer miapeMSIID = Integer
										.valueOf(cpMsi.getId());
								cpMsi.setName(FileManager
										.getMiapeMSIFileName(miapeMSIID));
								File file = null;
								if (curated) {
									file = new File(
											FileManager
													.getMiapeMSICuratedXMLFilePath(
															miapeMSIID,
															cpExperiment
																	.getName()));
									if (!file.exists())
									...
									file = new File(
											FileManager
													.getMiapeMSIXMLFilePath(miapeMSIID));
								}
								if (addNode) {
									jTreeProject.addNewNode(cpMsi,
											replicateNode);
								...
								if (MainFrame.userName != null
									...
									MiapeRetrieverManager.getInstance(
											MainFrame.userName,
											MainFrame.password).addRetrieving(
											miapeMSIID, "MSI", this);
							} else {
								// LOCAL MIAPE
								boolean addNode = true;
								cpMsi.setName(cpMsi.getName());
								File file = new File(
										FileManager.getMiapeMSIXMLFileLocalPath(
												cpMsi.getId(),
							...
							cpMsi.setName(cpMsi.getName());
							File file = FileManager.getManualIdSetFile(cpMsi
									.getName());
	...
	private void addExperimentToProjectTree(File projectFile) {
		final DefaultMutableTreeNode experimentListNode = jTreeProject
				.getRootNode();
		CPExperiment CPExperiment = new ExperimentAdapter(projectFile)
				.getCpExperiment();
		CPExperiment.setCurated(true);
		CPExperimentList cpExperimentList = (CPExperimentList) experimentListNode
				.getUserObject();
		...
		final DefaultMutableTreeNode experimentNode = jTreeProject.addNewNode(
				CPExperiment, experimentListNode);
		for (CPReplicate cpReplicate : CPExperiment.getCPReplicate()) {
			...
			if (cpReplicate.getCPMSIList() != null) {
				for (CPMSI cpMsi : cpReplicate.getCPMSIList().getCPMSI()) {
					...
					jTreeProject.addNewNode(cpMsi, replicateNode);
					cpMsi.setName(FilenameUtils.getName(FileManager
							.getMiapeMSICuratedXMLFilePath(cpMsi.getId(),
									experimentName)));

					final Integer miapeMSIID = Integer.valueOf(cpMsi.getId());
					File file = new File(
							FileManager.getMiapeMSICuratedXMLFilePath(
									miapeMSIID, experimentName));
					// if the file was created/modified before to 3 October
					// 2012:
					if (MainFrame.userName != null
							&& MainFrame.password != null
							&& (!file.exists() || file.lastModified() < Long
									.valueOf("1349251004048")))
						MiapeRetrieverManager.getInstance(MainFrame.userName,
								MainFrame.password).addRetrieving(miapeMSIID,
								"MSI", this);
				}
			}
		}
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				...
				Miape2ExperimentListDialog dialog = new Miape2ExperimentListDialog(
						null);
	...
	public synchronized void propertyChange(PropertyChangeEvent evt) {
		...
		// }
			...
			if (message.contains(MESSAGE_SPLITTER)) {
				...
				appendStatus("Error loading MIAPE " + miapeType + " " + miapeID
						+ ": " + messageError);
			...
			int size = MiapeRetrieverManager.getInstance(MainFrame.userName,
					MainFrame.password).getSize();
			if (size < 1)
				jProgressBar.setIndeterminate(false);
			...
			Integer size = MiapeRetrieverManager.getInstance(
					MainFrame.userName, MainFrame.password).getSize();
			if (size < 1)
				jProgressBar.setIndeterminate(false);
			String message = (String) evt.getNewValue();
			if (message.contains(MiapeRetrieverTask.MESSAGE_SPLITTER)) {
				Object[] splitter = message
				...
				size = MiapeRetrieverManager.getInstance(MainFrame.userName,
						MainFrame.password).getSize();
				if (size < 1)
					jProgressBar.setIndeterminate(false);
				appendStatus("MIAPE "
				...
				if (size > 0) {
					...
					appendStatus("Currently downloading in background: "
							+ executingTasks + " from MIAPE repository.");
					// this.appendStatus("This task will be performed in backgroud.");
				}
			}
		} else if (MiapeTreeIntegrityCheckerTask.INTEGRITY_OK.equals(evt
				.getPropertyName())) {
			jProgressBar.setIndeterminate(false);
			try {
				save(false);
		...
		} else if (MiapeTreeIntegrityCheckerTask.INTEGRITY_ERROR.equals(evt
				.getPropertyName())) {
			jProgressBar.setIndeterminate(false);
			String message = (String) evt.getNewValue();
			final int option = JOptionPane.showConfirmDialog(this, message,
					"Error in comparison project", JOptionPane.YES_NO_OPTION);
			if (option == JOptionPane.YES_OPTION) {
				try {
					save(false);
					if (finishPressed)
			...
			log.debug(num + " MIAPE MSI documents loaded");
			...
			appendStatus("Loading MIAPE documents. It should take a minute. Please wait...");
			appendStatus("You can stop MIAPE MSI loading by clicking on 'Stop' button.");
			...
			jProgressBar.setIndeterminate(false);
			enableCancelTreeLoadinButton(false);
		} else if (TreeLoaderTask.TREE_LOADER_CANCELED.equals(evt
				.getPropertyName())) {
			jButtonStartLoading.setEnabled(true);
			jButtonCancelLoading.setEnabled(false);
		} else if (ExternalDataTreeLoaderTask.EXTERNAL_TREE_LOADER_FINISHED
				.equals(evt.getPropertyName())) {
			Integer numLoaded = (Integer) evt.getNewValue();
			appendStatus(numLoaded + " external identification list imported.");
		} else if (ExternalDataTreeLoaderTask.EXTERNAL_TREE_LOADER_ERROR
				.equals(evt.getPropertyName())) {
			appendStatus("Error loading external identification lists: "
					+ evt.getNewValue());
		} else if (LocalDataTreeLoaderTask.LOCAL_TREE_LOADER_FINISHED
				.equals(evt.getPropertyName())) {
			Integer numLoaded = (Integer) evt.getNewValue();
			appendStatus(numLoaded + " local MIAPE documents detected.");
		} else if (LocalDataTreeLoaderTask.LOCAL_TREE_LOADER_ERROR.equals(evt
				.getPropertyName())) {
			appendStatus("Error loading local MIAPE documents: "
					+ evt.getNewValue());
		}
		pack();
	}
	...
	private void showChartManager() {
		...
		if (showChartManager) {
			ChartManagerFrame chartManager = ChartManagerFrame.getInstance(
					this, currentCgfFile);
