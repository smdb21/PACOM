 ...
 * @author __USER__
 */
public class ExporterToPRIDEDialog extends javax.swing.JDialog implements
	...
	public ExporterToPRIDEDialog(Frame parent, ExperimentList experimentList) {
		...
		if (this.experimentList != null) {
			...
			if (numExperiments > 1)
				plural = "s";
			String text = "<html>This project contains " + numExperiments
					+ " experiment" + plural + ".<br>" + numExperiments
					+ " PRIDE XML files will be created";
			if (numExperiments > 1)
				text = text + "(one per each experiment)";
			text = text
					+ ".<br>Select the output folder and click on 'export' button.</html>";
		...
		// set button icons
		jButtonExport.setIcon(ImageManager
				.getImageIcon(ImageManager.PRIDE_LOGO));
		jButtonExport.setPressedIcon(ImageManager
				.getImageIcon(ImageManager.PRIDE_LOGO_CLICKED));
		jButtonCancel.setIcon(ImageManager.getImageIcon(ImageManager.STOP));
		jButtonCancel.setPressedIcon(ImageManager
	...
	private void initComponents() {
		...
		jPanel4Layout.setHorizontalGroup(jPanel4Layout.createParallelGroup(
				...
				jPanel4Layout
						...
						.addComponent(jLabelInformation,
								javax.swing.GroupLayout.DEFAULT_SIZE, 534,
		...
		java.awt.Dimension screenSize = java.awt.Toolkit.getDefaultToolkit()
				.getScreenSize();
		setBounds((screenSize.width - 602) / 2, (screenSize.height - 403) / 2,
				602, 403);
	}// </editor-fold>
	...
	private void jButtonCancelActionPerformed(java.awt.event.ActionEvent evt) {
		if (exporterTask != null
				&& exporterTask.getState() == StateValue.STARTED) {
	...
	private void exportToPRIDE() {
		...
		try {
			...
			boolean excludeNonMatchedSpectra = jCheckBoxExcludeNotMatchedSpectra
					.isSelected();
			boolean compressResultingFiles = jCheckBoxCompress.isSelected();
			boolean excludeNonConclusiveProteins = jCheckBoxExcludeNonConclusiveProteins
					.isSelected();
			...
			if (exporterTask == null
					|| !exporterTask.getState().equals(StateValue.STARTED)) {
				exporterTask = new PRIDEExporterTask(experimentList,
						outputFolder, addPeakList, excludeNonMatchedSpectra,
						compressResultingFiles, excludeNonConclusiveProteins);
				exporterTask.addPropertyChangeListener(this);
				exporterTask.execute();
			} else {
				appendStatus("MIAPE Extractor is currently exporting data to PRIDE XML.");
	...
	public void propertyChange(PropertyChangeEvent evt) {
		if (evt.getPropertyName().equals(
			...
			if (generatedFile != null && generatedFile.exists()) {
				appendStatus("PRIDE XML file created at: "
			...
			if (compressingFile != null && compressingFile.exists()) {
				appendStatus("Compressing PRIDE XML file: "
						+ FilenameUtils.getName(compressingFile
								.getAbsolutePath()) + " ("
			...
			File generatedFile = (File) evt.getNewValue();
			if (generatedFile != null && generatedFile.exists()) {
				appendStatus("PRIDE XML file already exists at: "
						+ FilenameUtils
