 ...
 * @author __USER__
 */
public class ExporterDialog extends javax.swing.JDialog implements
		PropertyChangeListener, ExporterManager {
	...
	public ExporterDialog(ChartManagerFrame parent, IdentificationSet idSet) {
		...
		if (idSet != null && parent.getFiltersDialog() != null
			...
			this.jCheckBoxIncludeDecoy.setEnabled(true);
			this.isFDRApplied = true;
		} else {
			this.isFDRApplied = false;
		}

		// Just enable if the protein sequences have not been retrieved before
		if (parent != null)
			this.jCheckBoxSearchForProteinSequence.setEnabled(!parent
		...
		this.jButtonCancel
				.setIcon(ImageManager.getImageIcon(ImageManager.STOP));
		this.jButtonExport.setIcon(ImageManager
				.getImageIcon(ImageManager.EXCEL_TABLE));
		this.pack();
	...
	private void initComponents() {
		...
		jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(
				javax.swing.BorderFactory.createEtchedBorder(), "Output file"));
		...
		jRadioButtonExportPeptides
				.setToolTipText("<html>Peptide information will be together with its assigned protein\n<br>\naccession, but no more protein information will be exported.</html>");
		jRadioButtonExportPeptides
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		jRadioButtonExportProteins
				.setToolTipText("Only protein information will be exported.");
		jRadioButtonExportProteins
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		jCheckBoxIncludeNonConclusiveProteins
				.setText("include NONCONCLUSIVE proteins");
		jCheckBoxIncludeNonConclusiveProteins
		...
		layout.setVerticalGroup(layout
				...
				.addGroup(
						layout.createSequentialGroup()
								.addContainerGap()
								.addComponent(jLabelNumPeptides,
										javax.swing.GroupLayout.PREFERRED_SIZE,
										16,
										javax.swing.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(
										javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addComponent(jPanelOptions,
										javax.swing.GroupLayout.PREFERRED_SIZE,
										163,
										javax.swing.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(
		...
		java.awt.Dimension screenSize = java.awt.Toolkit.getDefaultToolkit()
				.getScreenSize();
		setBounds((screenSize.width - 498) / 2, (screenSize.height - 419) / 2,
				498, 419);
	}// </editor-fold>
		// GEN-END:initComponents

	private void jRadioButtonExportProteinsActionPerformed(
			java.awt.event.ActionEvent evt) {
		if (this.jRadioButtonExportProteins.isSelected()) {
	...
	private void jRadioButtonExportPeptidesActionPerformed(
			java.awt.event.ActionEvent evt) {
		if (this.jRadioButtonExportPeptides.isSelected()) {
	...
	private void updateSizesLabels() {
		...
		if (this.jRadioButtonExportProteins.isSelected()) {
			if (this.jCheckBoxCollapseProteins.isSelected())
				sizeProteins = idSet
		...
		if (sizeProteins != null && !this.jCheckBoxIncludeDecoy.isSelected()) {
			if (this.jCheckBoxCollapseProteins.isSelected()) {
		...
		if (this.jRadioButtonExportPeptides.isSelected()) {
			if (this.jCheckBoxCollapsePeptides.isSelected())
				sizePeptides = idSet.getNumDifferentPeptides(true);
			else
				sizePeptides = idSet.getTotalNumPeptides();
		}
		if (sizePeptides != null && !this.jCheckBoxIncludeDecoy.isSelected()) {
			if (this.jCheckBoxCollapseProteins.isSelected())
		...
		this.jLabelNumPeptides
				.setText(getNumString(sizePeptides, sizeProteins));
	...
	private void jButtonExportActionPerformed(java.awt.event.ActionEvent evt) {
		...
		this.setControlStatusToDisabled();
		this.jButtonCancel.setEnabled(true);
		this.exporter = new TSVExporter(this, idSet, file);
	...
	public boolean isReplicateAndExperimentOriginIncluded() {
		return this.jCheckBoxIncludeExperimentReplicateOrigin.isSelected();
	}

	@Override
	public boolean isDecoyHitsIncluded() {
		return this.jCheckBoxIncludeDecoy.isSelected();
	}

	@Override
	public boolean showPeptides() {
		return this.jRadioButtonExportPeptides.isSelected();
	...
	public boolean isGeneInfoIncluded() {
		...
		return this.jCheckBoxIncludeGeneInfo.isSelected();

	}

	@Override
	public boolean retrieveProteinSequences() {
		return this.jCheckBoxSearchForProteinSequence.isSelected();
	}

	private void jButtonSelectActionPerformed(java.awt.event.ActionEvent evt) {
		this.jTextFieldFilePath.setText(selectFile());
	}
	...
	public String selectFile() {
		...
		this.jFileChooser1
				.setDialogTitle("Select an existing file or type the name for a new one");
		this.jFileChooser1.setFileFilter(new TsvFileFilter());
		this.jFileChooser1.showOpenDialog(this);
		if (this.jFileChooser1.getSelectedFile() != null) {
			filename = this.jFileChooser1.getSelectedFile().toString();
	...
	private File getFile() {
		final String path = this.jTextFieldFilePath.getText();
		validatePath(path);
	...
	private void validatePath(String path) {
		if ("".equals(path)) {
			JOptionPane.showMessageDialog(this, "Select the output path",
	...
	public void propertyChange(PropertyChangeEvent evt) {
		if (TSVExporter.DATA_EXPORTING_DONE.equals(evt.getPropertyName())) {
			...
			JOptionPane.showMessageDialog(this, "Data exported succesfully at "
					+ file.getAbsolutePath(), "Data exported",
		...
		} else if (TSVExporter.DATA_EXPORTING_ERROR.equals(evt
				.getPropertyName())) {
			JOptionPane.showMessageDialog(this,
					"Error exporting data: " + evt.getNewValue(),
					"Error exporting data", JOptionPane.INFORMATION_MESSAGE);
			setControlStatusToPrevious();
			this.jButtonCancel.setEnabled(false);
			this.jButtonExport.setEnabled(true);

		} else if (TSVExporter.DATA_EXPORTING_CANCELED.equals(evt
				.getPropertyName())) {
			this.jProgressBar1.setValue(0);
			setControlStatusToPrevious();
			this.jButtonCancel.setEnabled(false);
			this.jButtonExport.setEnabled(true);
		} else if (TSVExporter.DATA_EXPORTING_SORTING.equals(evt
				.getPropertyName())) {
			int size = (Integer) evt.getNewValue();
			this.jProgressBar1.setIndeterminate(true);
			this.jProgressBar1.setString("Sorting " + size
					+ " items by score...");
			this.jProgressBar1.setStringPainted(true);
		} else if (TSVExporter.DATA_EXPORTING_SORTING_DONE.equals(evt
				.getPropertyName())) {
	...
	private void setControlStatusToPrevious() {
		this.jCheckBoxCollapsePeptides
				.setEnabled(this.previousCollapsePeptides);
		this.jCheckBoxCollapseProteins
				.setEnabled(this.previousCollapseProteins);
		this.jCheckBoxIncludeDecoy.setEnabled(this.previousIncludeDecoy);
		this.jCheckBoxIncludeExperimentReplicateOrigin
				.setEnabled(this.previousIncludeExperimentReplicate);
		this.jCheckBoxIncludeGeneInfo.setEnabled(this.previousIncludeGene);
		this.jCheckBoxSearchForProteinSequence
				.setEnabled(this.previousSearchForProteinSequence);
		this.jButtonExport.setEnabled(true);
		this.jRadioButtonExportPeptides.setEnabled(true);
		this.jRadioButtonExportProteins.setEnabled(true);
		this.jCheckBoxIncludeNonConclusiveProteins
				.setEnabled(this.previousExcludeNonConclusive);

	}

	private void setControlStatusToDisabled() {
		this.previousCollapsePeptides = this.jCheckBoxCollapsePeptides
				.isEnabled();
		this.previousCollapseProteins = this.jCheckBoxCollapseProteins
				.isEnabled();
		this.previousIncludeDecoy = this.jCheckBoxIncludeDecoy.isEnabled();
		this.previousIncludeExperimentReplicate = this.jCheckBoxIncludeExperimentReplicateOrigin
				.isEnabled();
		this.previousIncludeGene = this.jCheckBoxIncludeGeneInfo.isEnabled();
		this.previousSearchForProteinSequence = this.jCheckBoxSearchForProteinSequence
				.isEnabled();
		this.previousExcludeNonConclusive = this.jCheckBoxIncludeNonConclusiveProteins
				.isEnabled();

		this.jCheckBoxCollapsePeptides.setEnabled(false);
		this.jCheckBoxCollapseProteins.setEnabled(false);
		this.jCheckBoxIncludeDecoy.setEnabled(false);
		this.jCheckBoxIncludeExperimentReplicateOrigin.setEnabled(false);
		this.jCheckBoxIncludeGeneInfo.setEnabled(false);
		this.jCheckBoxSearchForProteinSequence.setEnabled(false);
	...
	public boolean showBestPeptides() {
		return this.jCheckBoxCollapsePeptides.isSelected();
	}

	@Override
	public boolean showBestProteins() {
		return this.jCheckBoxCollapseProteins.isSelected();
	}

	@Override
	public boolean isNonConclusiveProteinsIncluded() {
		return this.jCheckBoxIncludeNonConclusiveProteins.isSelected();
	}

	@Override
	public boolean isFDRApplied() {
		return this.isFDRApplied;
	}
