...
public class GeneralOptionsDialog extends javax.swing.JDialog {
	...
	public static GeneralOptionsDialog getInstance(ChartManagerFrame parent) {
		...
		instance.previousGroupAtExperimentListLevel = instance
				.groupProteinsAtExperimentListLevel();
		instance.previousLocalProcessingInParallel = instance
	...
	public void restoreDefaultValues() {
		this.jTextFieldPeptideLength.setText(String
				.valueOf(DataManager.DEFAULT_MIN_PEPTIDE_LENGTH));
	}

	public boolean isDoNotAskAgainSelected() {
		return this.jCheckBoxDoNotAskAgain.isSelected();
	}
	...
	public void dispose() {
		...
		final boolean dataShouldBeLoaded = this.parent.dataShouldBeLoaded(
				this.parent.cfgFile, groupAtExperimentListLevel,
				minPeptideLength, localProcessingInParallel);
		if (dataShouldBeLoaded)
			this.parent.loadData(minPeptideLength, groupAtExperimentListLevel,
	...
	public boolean groupProteinsAtExperimentListLevel() {
		return this.jCheckBoxGroupProteinsInExperimentList.isSelected();
	}

	public boolean isLocalProcessingInParallel() {
		return this.jCheckBoxProcessInParallel.isSelected();
	}
	...
	private GeneralOptionsDialog(ChartManagerFrame parent) {
		...
		initComponents();
		this.setTitle("General Options");
		this.parent = parent;
		this.jLabelWarning.setVisible(false);
		RefineryUtilities.centerFrameOnScreen(this);

		String text = getCoresToUseText();
		this.jLabelUsingCores.setText(text);
	}

	private String getCoresToUseText() {
		final int availableProcessors = Runtime.getRuntime()
				.availableProcessors();
		if (this.jCheckBoxProcessInParallel.isSelected()) {
			final int availableNumSystemCores = SystemCoreManager
					.getAvailableNumSystemCores(MAXIMUM_NUM_CORES);
			return "Using " + availableNumSystemCores + " out of "
	...
	private void initComponents() {
		...
		jLabelWarning
				.setText("<html><b>Note</b>: Any change here will cause project data reloading.</html>");
		...
		jLabel1.setToolTipText("<html>Peptides with short sequences can be somehow problematic, <br> since short decoy sequences could have similar identification <br> scores than a non decoy short sequence.</html>");

		jTextFieldPeptideLength
				.setHorizontalAlignment(javax.swing.JTextField.CENTER);
		jTextFieldPeptideLength.setText("7");
		jTextFieldPeptideLength
				.setToolTipText("<html>Peptides with short sequences can be somehow problematic, <br>\nsince short decoy sequences could have similar identification <br>\nscores than a non decoy short sequence.</html>");
		...
		jLabel2.setText("AA");
		jLabel2.setToolTipText("<html>Peptides with short sequences can be somehow problematic, <br> since short decoy sequences could have similar identification <br> scores than a non decoy short sequence.</html>");
		...
		jPanel4Layout.setHorizontalGroup(jPanel4Layout.createParallelGroup(
				...
				jPanel4Layout
						.createSequentialGroup()
						.addContainerGap()
						.addComponent(jCheckBoxProcessInParallel)
						.addGap(18, 18, 18)
						.addComponent(jLabelUsingCores,
						...
						.addGroup(
								jPanel5Layout
										...
										.addPreferredGap(
												javax.swing.LayoutStyle.ComponentPlacement.RELATED,
												205, Short.MAX_VALUE)
										.addComponent(jCheckBoxDoNotAskAgain)
										.addContainerGap()));
		jPanel5Layout
	...
	private void jCheckBoxProcessInParallelActionPerformed(
			java.awt.event.ActionEvent evt) {

		this.jLabelUsingCores.setText(getCoresToUseText());

		if (!String.valueOf(this.jCheckBoxProcessInParallel.isSelected())
				.equals(String.valueOf(this.previousLocalProcessingInParallel)))
			showWarningLabel(true);
		else
			showWarningLabel(false);
	}

	private void jCheckBoxGroupProteinsInExperimentListActionPerformed(
			java.awt.event.ActionEvent evt) {
		if (!String
				.valueOf(
						this.jCheckBoxGroupProteinsInExperimentList
								.isSelected())
				.equals(String.valueOf(this.previousGroupAtExperimentListLevel)))
			showWarningLabel(true);
		else
			showWarningLabel(false);
	}
	...
	private void jTextFieldPeptideLengthKeyReleased(java.awt.event.KeyEvent evt) {
		...
		try {
			min = Integer.valueOf(this.jTextFieldPeptideLength.getText());
		} catch (NumberFormatException e) {

		}
		if (!min.equals(this.previousMinPeptideLength))
			showWarningLabel(true);
		else {
			showWarningLabel(false);
		}
	...
	private void showWarningLabel(boolean show) {
		this.jLabelWarning.setVisible(show);
		this.pack();
	}
