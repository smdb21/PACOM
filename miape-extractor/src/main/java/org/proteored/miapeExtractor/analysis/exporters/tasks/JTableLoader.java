...
public class JTableLoader extends SwingWorker<Void, Void> implements
		Exporter<JTable> {
	...
	public JTableLoader(ExporterManager manager, IdentificationSet idSet,
			JTable table2) {
		...
		this.includeReplicateAndExperimentOrigin = manager
		...
		this.excludeNonConclusiveProteins = !manager
	...
	public JTableLoader(ExporterManager manager, IdentificationSet idSet,
			JTable table, boolean useTestOntologies) {
		this(manager, idSet, table);
		...
		try {
			...
			List<String> columnsStringList = ExportedColumns
					.getColumnsStringForTable(
							...
							this.includePeptides, this.includeGeneInfo,
							this.isFDRApplied, this.idSet);
			...
			if (this.includePeptides) {
				if (this.collapsePeptides) {
					...
					while (iterator.hasNext()) {
						...
						if (excludeNonConclusiveProteins
								&& ExporterUtil
										.isNonConclusivePeptide(peptideOccurrence))
							pass = false;
						if (pass
							...
							final List<String> lineStringList = ExporterUtil
									.getInstance(
											...
											includePeptides, includeGeneInfo,
					...
					for (ExtendedIdentifiedPeptide peptide : identifiedPeptides) {
						...
						boolean pass = true;
						if (excludeNonConclusiveProteins
								&& ExporterUtil.isNonConclusivePeptide(peptide))
							pass = false;
						if (pass
								&& (includeDecoyHits || (!includeDecoyHits && !isdecoy))) {
				...
				if (this.collapseProteins) {
					...
					// sort if there is a FDR Filter activated that tells us
					// which is the score sort
					firePropertyChange(DATA_EXPORTING_STARTING, null,
							proteinOccurrenceList.size());
					firePropertyChange(DATA_EXPORTING_SORTING, null,
							proteinOccurrenceList.size());
					try {
						SorterUtil
								.sortProteinGroupOcurrencesByBestPeptideScore(proteinOccurrenceList);
					} catch (Exception e) {
						log.info(e.getMessage());
					}
					firePropertyChange(DATA_EXPORTING_SORTING_DONE, null, null);
					Iterator<ProteinGroupOccurrence> iterator = proteinOccurrenceList
							.iterator();
					...
					for (ProteinGroup proteinGroup : proteinGroups) {
						...
						if (excludeNonConclusiveProteins
								&& ExporterUtil
										.isNonConclusiveProtein(proteinGroup))
							pass = false;
						if (pass
								&& (includeDecoyHits || (!includeDecoyHits && !isdecoy))) {
	...
	protected void done() {
		...
		if (!this.isCancelled())
			firePropertyChange(DATA_EXPORTING_DONE, null,
