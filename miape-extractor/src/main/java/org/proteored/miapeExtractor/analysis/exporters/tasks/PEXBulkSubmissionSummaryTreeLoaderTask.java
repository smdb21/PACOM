...
public class PEXBulkSubmissionSummaryTreeLoaderTask extends
	...
	private static Set<String> validatedURLs = new HashSet<String>();

	public PEXBulkSubmissionSummaryTreeLoaderTask(
			ExperimentList experimentList, ExtendedJTree jTreeSummary,
		...
		try {
			...
			if (jTreeSummary != null && expList != null) {
				...
				if (expList.getExperiments() != null) {
					...
					int num = 0;
					HashMap<Replicate, List<String>> msiFilesByReplicate = expList
							.getMSIGeneratedFilesByReplicate();
					HashMap<Replicate, List<ResultingData>> rawFilesByReplicate = expList
							.getRawFileResultingDataMapByReplicate();
					HashMap<Replicate, List<ResultingData>> peaklistFilesByReplicate = expList
					...
					for (Experiment exp : expList.getExperiments()) {
						...
						if (exp.getReplicates() != null) {
							for (Replicate rep : exp.getReplicates()) {
								...
								Thread.sleep(1L);
								num++;
								int progress = Math.round(100 * num
										/ numReplicates);
								log.info("Progress: " + progress);
								setProgress(progress);
								DefaultMutableTreeNode repNode = new DefaultMutableTreeNode(
										rep);

								List<DefaultMutableTreeNode> searchNodes = new ArrayList<DefaultMutableTreeNode>();
								// Attached MSI files
								if (includeMSIAttachedFiles
										&& msiFilesByReplicate != null
										&& msiFilesByReplicate.containsKey(rep)) {
									List<String> msiAttachedFiles = msiFilesByReplicate
											.get(rep);
									if (msiAttachedFiles != null) {
										for (String msiAttachedFileLocation : msiAttachedFiles) {
											try {
												final URL url = new URL(
														msiAttachedFileLocation);
								...
								if (peaklistFilesByReplicate != null
									...
									if (resultingDatas != null) {
										for (ResultingData resultingdata : resultingDatas) {
											try {
												String dataFileUri = resultingdata
														.getDataFileUri();
								...
								if (rawFilesByReplicate != null
										&& rawFilesByReplicate.containsKey(rep)) {
									...
									if (resultingDatas != null) {
										for (ResultingData resultingdata : resultingDatas) {
											try {
												...
												// data
												firePropertyChange(
														BULK_SUBMISSION_TREE_LOADER_RAW_DATA_PRESENT,
														null, null);
											} catch (MalformedURLException e) {
												// do nothing
											}
								...
								if (includeMIAPEReports) {
									// MIAPE MS documents
									List<MiapeMSDocument> miapeMSs = rep
											.getMiapeMSs();
									if (miapeMSs != null) {
										for (MiapeMSDocument miapeMSDocument : miapeMSs) {
											...
											if (miapeMSDocument.getId() > 0) {
												DefaultMutableTreeNode miapeMSNode = new DefaultMutableTreeNode(
														OTHER
																+ ": MIAPE_MS_"
																+ miapeMSDocument
																		.getId()
									...
									// MIAPE MSI documents
									List<MiapeMSIDocument> miapeMSIs = rep
											.getMiapeMSIs();
									if (miapeMSs != null) {
										for (MiapeMSIDocument miapeMSIDocument : miapeMSIs) {
											if (miapeMSIDocument.getId() > 0) {
												DefaultMutableTreeNode miapeMSINode = new DefaultMutableTreeNode(
														OTHER
																+ ": MIAPE_MSI_"
																+ miapeMSIDocument
																		.getId()
						...
						firePropertyChange(
								BULK_SUBMISSION_TREE_LOADER_EXPERIMENT_LOADED,
								null, exp);
					}
				}
			...
			firePropertyChange(BULK_SUBMISSION_TREE_LOADER_FINISH, null,
					e.getMessage());
		}
		return null;
	}
	...
	public static void main(String[] args) {
		try {
			boolean valid = URLValidator
					.validateURL(new URL(
							"ftp://pme6@estrellapolar.cnb.csic.es/data/CBT/PME6_I_CBT_Orbitrap.raw"));
