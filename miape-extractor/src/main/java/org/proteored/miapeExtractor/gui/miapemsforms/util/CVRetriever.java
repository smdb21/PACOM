package org.proteored.miapeExtractor.gui.miapemsforms.util;
...
public class CVRetriever {
	...
	private List<String> getNames(List<ControlVocabularyTerm> terms) {
		...
		Set<String> names = new HashSet<String>(); // not allow repeated values
		if (terms != null) {
			for (ControlVocabularyTerm controlVocabularyTerm : terms) {
				final String preferredName = controlVocabularyTerm
						.getPreferredName();
				if (preferredName.length() < 70
						&& !names.contains(preferredName)) {
	...
	public List<String> getAnalyzerNames() {
		final List<ControlVocabularyTerm> analyzerNames = MassAnalyzerType
				.getInstance(cvManager).getPossibleValues();
		return getNames(analyzerNames);
	}

	public List<String> getManufacturerNames() {
		final List<ControlVocabularyTerm> possibleValues = InstrumentVendor
				.getInstance(cvManager).getPossibleValues();
	...
	public List<String> getInstrumentNames() {
		...
		final List<ControlVocabularyTerm> instrumentNames = InstrumentModel
				.getInstance(cvManager).getPossibleValues();
	...
	public List<String> getRelectronStates() {
		final List<ControlVocabularyTerm> reflectronStates = ReflectronState
				.getInstance(cvManager).getPossibleValues();
		return getNames(reflectronStates);
	}

	public List<String> getEsiNames() {
		final List<ControlVocabularyTerm> esiNames = EsiName.getInstance(
				cvManager).getPossibleValues();
		return getNames(esiNames);
	}

	public List<String> getSupplyTypeNames() {
		final List<ControlVocabularyTerm> esiNames = SupplyType.getInstance(
				cvManager).getPossibleValues();
		return getNames(esiNames);
	}

	public List<String> getMaldiDissociations() {
		final List<ControlVocabularyTerm> ret = MaldiDissociationMethod
				.getInstance(cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getLaserTypes() {
		final List<ControlVocabularyTerm> ret = LaserType
				.getInstance(cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getOtherIonSourceNames() {
		final List<ControlVocabularyTerm> ret = IonSourceName.getInstance(
				cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getGasTypes() {
		final List<ControlVocabularyTerm> ret = GasType.getInstance(cvManager)
				.getPossibleValues();
		return getNames(ret);
	}

	public List<String> getPressureUnits() {
		final List<ControlVocabularyTerm> ret = PressureUnit.getInstance(
				cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getActivationTypes() {
		final List<ControlVocabularyTerm> ret = DissociationMethod.getInstance(
				cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getSoftwareAcquisitionNames() {
		final List<ControlVocabularyTerm> ret = AcquisitionSoftware
				.getInstance(cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getDataAnalysisSoftwareNames() {
		final List<ControlVocabularyTerm> ret = DataProcessingSoftware
				.getInstance(cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getDataAnalysisDescription() {
		final List<ControlVocabularyTerm> ret = DataProcessingAction
				.getInstance(cvManager).getPossibleValues();
		return getNames(ret);
	}
	...
	public List<String> getAddInformations() {
		if (!addInfos.isEmpty())
			return addInfos;
		final List<String> addInfos = getNames(AdditionalInformationName
				.getInstance(cvManager).getPossibleValues());
		...
		for (String string : addInfos) {
			if (!tissueNames.contains(string) && !cellTypes.contains(string)
					&& !diseaseNames.contains(string)
					&& !samplePrepNames.contains(string)
					&& !taxonomyNames.contains(string))
				ret.add(string);
		}
	...
	public List<String> getSampleProcessingSteps() {
		final List<ControlVocabularyTerm> ret = SampleProcessingStep
				.getInstance(cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getSampleProperties() {
		final List<ControlVocabularyTerm> ret = SampleInformation.getInstance(
				cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getTissueNames() {
		final List<ControlVocabularyTerm> ret = TissuesTypes.getInstance(
				cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getCellTypesNames() {
		final List<ControlVocabularyTerm> ret = CellTypes
				.getInstance(cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getDisseasesNames() {
		final List<ControlVocabularyTerm> ret = HumanDisseases.getInstance(
				cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getPositions() {
		final List<ControlVocabularyTerm> ret = ContactPositionMS.getInstance(
				cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getSamplePlateType() {
		final List<ControlVocabularyTerm> ret = SamplePlateType.getInstance(
				cvManager).getPossibleValues();
		return getNames(ret);
	}

	public List<String> getTaxonomies() {
		final List<ControlVocabularyTerm> ret = MainTaxonomies.getInstance(
				cvManager).getPossibleValues();
		return getNames(ret);
	}
}
