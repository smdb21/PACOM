...
public class ExperimentsUtilTest {
	...
	private static SortingParameters defaultSortingProteins = new SortingParameters(
			PROTEIN_SCORE_NAME, PROTEIN_SCORE_ORDER);
	private static SortingParameters defaultSortingPeptides = new SortingParameters(
			PEPTIDE_SCORE_NAME, PEPTIDE_SCORE_ORDER);
	...
	public static ExperimentList createExperiments(List<Filter> filters) {
		...
		return new ExperimentList("Experiment list", experimentList, false,
				filters, minPeptideLength, null);
	}

	public static Experiment createExperiment(String experimentName,
			List<Filter> filters) {

		final List<Replicate> replicates = createReplicates(experimentName,
				filters);
		Experiment experiment = new Experiment(experimentName, replicates,
				filters, minPeptideLength, null);

		return experiment;
	}

	private static List<Replicate> createReplicates(String experimentName,
			List<Filter> filters) {
	...
	public static Replicate createReplicate(String experimentName,
			String replicateName, List<Filter> filters) {
		final MiapeMSIDocument createMiapeMSI = createMiapeMSI(experimentName,
				replicateName);
		List<MiapeMSIDocument> miapeList = new ArrayList<MiapeMSIDocument>();
		miapeList.add(createMiapeMSI);
		return new Replicate(replicateName, experimentName, null, miapeList,
				filters, null, null);
	}

	private static MiapeMSIDocument createMiapeMSI(String experimentName,
			String replicateName) {
		Project project = MiapeDocumentFactory.createProjectBuilder(
				"ReplicateTestProject").build();
		final MiapeMSIDocumentBuilder msiBuilder = MiapeMSIDocumentFactory
				.createMiapeDocumentMSIBuilder(project, experimentName + " "
						+ replicateName, null, null, null, null);
		msiBuilder.identifiedProteinSets(createIdentifiedProteinSet());
		msiBuilder
				.identifiedPeptides(createIdentifiedPeptides(MAX_NUM_PEPTIDES_TOTAL));
	...
	private static Set<IdentifiedProteinSet> createIdentifiedProteinSet() {
		...
		for (IdentifiedProtein identifiedProtein : createIdentifiedProteins) {
			hashproteins.put(identifiedProtein.getAccession(),
					identifiedProtein);
		}
		identifiedProteinSet.add(MiapeMSIDocumentFactory
				.createIdentifiedProteinSetBuilder("protein set")
				.identifiedProteins(hashproteins).build());
		return identifiedProteinSet;
	}

	public static List<ExtendedIdentifiedProtein> createIdentifiedProteins(
			int number, boolean createAlsoPeptides) {
		...
		for (int i = 0; i < number; i++) {
			...
			scores.add(new ProteinScoreBuilder(PROTEIN_SCORE_NAME, random)
					.build());
			String prefix = "";
			...
			final IdentifiedProtein protein = MiapeMSIDocumentFactory
					.createIdentifiedProteinBuilder(
							prefix + "Protein" + getRandomInt(number / 2))
					.scores(scores).identifiedPeptides(peptides)
					.id(identifier++).build();

			identifiedProteins.add(new ExtendedIdentifiedProtein(null, protein,
					null));
		}
	...
	public static List<IdentifiedPeptide> createIdentifiedPeptides(int number) {
		...
		for (int i = 0; i < number; i++) {
			...
			final String random = getRandomDouble().toString();
			scores.add(new PeptideScoreBuilder(PEPTIDE_SCORE_NAME, random)
					.build());
			...
			final IdentifiedPeptide peptide = MiapeMSIDocumentFactory
					.createIdentifiedPeptideBuilder(sequence).score(scores)
					.identifiedProteins(proteins).id(identifier++).build();
	...
	public static FDRFilter getFDRFilter(float threshold,
			boolean concatenatedDecoySearch, SortingParameters sorting,
			IdentificationItemEnum item) {
		return new FDRFilter(threshold, getDecoyPrefix(),
				concatenatedDecoySearch, sorting, item, null, null, null);
	}
	...
	public static void printProteinList(List<ProteinGroup> identifiedProteins) {
		...
		for (ProteinGroup identifiedProtein : identifiedProteins) {
			System.out.print(identifiedProtein.getAccessions().iterator()
					.next()
					+ ":"
					+ identifiedProtein.getScores().iterator().next()
							.getValue() + " ");
		}
		System.out.println("----");
	}
