...
public class RemoteTests {
	...
	public void initialize() {
		...
		try {
			...
			miapeExtractorEndPoint = new URL(prop.getProperty("miapeextractor.webservice.endpoint"));
			log.info("Reading properties file: miapeconverter.webservice.endpoint: " + miapeExtractorEndPoint);

			miapeAPIEndPoint = new URL(prop.getProperty("miapeapi.webservice.endpoint"));
			log.info("Reading properties file: miapeapi.webservice.endpoint: " + miapeAPIEndPoint);

			emailNotifications = Boolean.parseBoolean(prop.getProperty("email_notifications"));
			log.info("Reading properties file: email_notifications: " + emailNotifications);

			String miapeAPIServiceName = prop.getProperty("miapeapi.webservice.servicename");
			log.info("Reading properties file: miapeAPIServiceName: " + miapeAPIServiceName);

			String miapeAPINameSpace = prop.getProperty("miapeapi.webservice.namespaceURI");
			log.info("Reading properties file: miapeAPINameSpace: " + miapeAPINameSpace);

			String miapeExtractorServiceName = prop.getProperty("miapeextractor.webservice.servicename");
			log.info("Reading properties file: miapeExtractorServiceName: " + miapeExtractorServiceName);

			String miapeExtractorNameSpace = prop.getProperty("miapeextractor.webservice.namespaceURI");
			log.info("Reading properties file: miapeExtractorNameSpace: " + miapeExtractorNameSpace);

			// Create the miape_api webservice proxy
			miapeAPIWebservice = new MiapeAPIWebserviceService(miapeAPIEndPoint,
					new QName(miapeAPINameSpace, miapeAPIServiceName)).getMiapeAPIWebservicePort();
					// ((BindingProvider)
					// miapeAPIWebservice).getRequestContext().put(
					// BindingProviderProperties.REQUEST_TIMEOUT,
					// WEBSERVICE_TIMEOUT);

			// Create the miape_generator webservice proxy
			System.out.println("miapeExtractorEndPoint=" + miapeExtractorEndPoint);
			miapeExtractorWebservice = new MiapeExtractorService(miapeExtractorEndPoint,
					new QName(miapeExtractorNameSpace, miapeExtractorServiceName)).getMiapeExtractorPort();
	...
	private Properties getProperties() throws Exception {
		...
		if (is == null)
			throw new Exception(MIAPE_CONVERTER_PROPERTIES_FILE + " file not found");
	...
	public void test1() {
		try {
			...
			Double min = null;
			Double max = null;
			List<IdentificationSet> listOfIdSets = Util.getListOfIdSets(experiments.getExperiments());
	...
	private ExperimentList createExperimentFromMiapeProject(int idProject)
			throws IOException, JAXBException, MiapeDatabaseException_Exception, MiapeSecurityException_Exception {

		int[] doneList = {};

		final HashMap<Integer, String> miapesByProjectID = Wrapper
				.getHashMap(miapeAPIWebservice.getMiapesByProjectID(idProject, userName, password));
		SortingParameters sprot = new SortingParameters("Mascot:score", Order.DESCENDANT);
		SortingParameters spep = new SortingParameters("Mascot:score", Order.DESCENDANT);
		...
		for (Integer miapeId : miapesByProjectID.keySet()) {
			...
			if (miapeType.equals("MSI")) {
				if (!isInList(miapeId, doneList)) {
					...
					Replicate replicate = new Replicate(miapeMSI.getName(), miapeMSI.getName(), null, listMiapeMSI,
							null, null, cvManager, false);
			...
			if (i == 2) {
				Experiment experiment = new Experiment(replicates.iterator().next().getName(), replicates, null,
						minPeptideLength, cvManager, false);
		...
		ExperimentList expList = new ExperimentList("PME6", experiments, false, null, minPeptideLength, cvManager,
				false);
		return null;
	...
	private MiapeMSDocument getMIAPEMS(Integer miapeId)
			throws JAXBException, IOException, MiapeDatabaseException_Exception, MiapeSecurityException_Exception {
		log.info("Retrieving MIAPE MS " + miapeId);
		byte[] miapeMSBytes = miapeAPIWebservice.getMiapeMSById(miapeId, userName, password);
		log.info("MIAPE MS " + miapeId + " retrieved.");
		MiapeXmlFile<MiapeMSDocument> miapeMSFile = new MIAPEMSXmlFile(miapeMSBytes);

		final MiapeMSDocument miapeMS = MiapeMSXmlFactory.getFactory().toDocument(miapeMSFile, cvManager, null,
				userName, password);
		miapeMSFile.saveAs(path + miapeMS.getName() + ".xml");
		return miapeMS;
	}

	private MiapeMSIDocument getMIAPEMSI(Integer miapeId)
			throws JAXBException, IOException, MiapeDatabaseException_Exception, MiapeSecurityException_Exception {
		log.info("Retrieving MIAPE MSI " + miapeId);
		long t1 = System.currentTimeMillis();
		byte[] miapeMSIBytes = miapeAPIWebservice.getMiapeMSIById(miapeId, userName, password);
		log.info("MIAPE MSI " + miapeId + " retrieved.");
		log.info("Loaded in " + (System.currentTimeMillis() - t1) / 1000 + " sg.");
		MiapeXmlFile<MiapeMSIDocument> miapeMSIFile = new MIAPEMSIXmlFile(miapeMSIBytes);
		final ControlVocabularyManager cvManager = SpringHandler.getInstance().getCVManager();
		final MiapeMSIDocument miapeMSI = MiapeMSIXmlFactory.getFactory().toDocument(miapeMSIFile, cvManager, null,
				userName, password);
		miapeMSIFile.saveAs(path + "MIAPE_MSI_" + miapeId + ".xml");
		return miapeMSI;
	}
}
