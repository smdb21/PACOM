...
public class PRIDEExporterTask extends SwingWorker<Void, Void> implements
		Exporter<List<File>> {
	...
	public PRIDEExporterTask(ExperimentList experimentList, File outputFolder,
			boolean addPeakLists, boolean removeNotMatchedSpectra,
			boolean compressResultingFiles, boolean excludeNonConclusiveProteins) {
	...
	public List<File> export() {
		...
		try {
			...
			for (Experiment experiment : this.experimentList.getExperiments()) {
				try {
					String newFilePath = outputFolder.getAbsolutePath()
							+ File.separator + experiment.getName() + ".xml";
					File xmlFile = new File(newFilePath);
					if (xmlFile.exists()) {
						firePropertyChange(SINGLE_PRIDE_ALREADY_PRESENT, null,
								xmlFile);

						if (compressResultingFiles) {
							firePropertyChange(
									SINGLE_PRIDE_COMPRESSING_STARTED, null,
									xmlFile);
							File compressedXmlFile = ZipManager
									.compressGZipFile(xmlFile);
							xmlFile.delete();
							firePropertyChange(
									SINGLE_PRIDE_COMPRESSING_FINISHED, null,
									compressedXmlFile);
					...
					firePropertyChange(SINGLE_PRIDE_EXPORTING_STARTED, null,
							experiment.getFullName());
					log.info("output file for experiment: "
							+ xmlFile.getAbsolutePath());
					ExperimentCollection experimentCollection = new ExperimentCollectionAdapterFromExperiment(
							experiment, OntologyLoaderTask.getCvManager(),
							this.addPeakLists, this.removeNotMatchedSpectra,
							this.excludeNonConclusiveProteins,
							"ProteoRed MIAPE Extractor v"
									+ MainFrame.getVersion()).adapt();
					log.info("adapted to XML: " + experimentCollection);

					marshaller.marshal(experimentCollection, xmlFile);

					log.info("created file: " + xmlFile.getAbsolutePath()
							+ " - " + xmlFile.length() / 1024 / 1024 + "Mbytes");

					if (compressResultingFiles) {
						firePropertyChange(SINGLE_PRIDE_COMPRESSING_STARTED,
								null, xmlFile);
						File compressedXmlFile = ZipManager
								.compressGZipFile(xmlFile);
						xmlFile.delete();
						firePropertyChange(SINGLE_PRIDE_COMPRESSING_FINISHED,
								null, compressedXmlFile);
						firePropertyChange(SINGLE_PRIDE_EXPORTED, null,
								compressedXmlFile);
						experiment.setPrideXMLFile(compressedXmlFile);
						generatedFiles.add(compressedXmlFile);
					} else {
					...
					firePropertyChange(SINGLE_PRIDE_EXPORTED_ERROR, null,
							e.getMessage());
				}
			}
