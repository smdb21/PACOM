...
public class MiapeExtractionTask extends SwingWorker<Void, Void> {
	...
	private MiapeExtractionResult result;
	...
	private boolean checkInputs() {
		...
		if (miapeExtractorInputParameters.isMzIdentMLPlusMGFSelected()) {
			if ("".equals(miapeExtractorInputParameters.getAssociatedMSFileName())) {
				firePropertyChange(MIAPE_CREATION_ERROR, null,
		...
		if (miapeExtractorInputParameters.isMzMLPlusMzIdentMLSelected()
			...
			if ("".equals(miapeExtractorInputParameters.getInputFileName())) {
				firePropertyChange(MIAPE_CREATION_ERROR, null,
	...
	private void convertFile2MIAPE()
		...
		} else if (miapeExtractorInputParameters.isProteinPilotSelected()) {
			extractMIAPEfromProteinPilotFile(tinnyMiapeMS);
	...
	private void extractMIAPEfromMzMLAndMzIdentMLFile(boolean fastParser, MiapeXmlFile<MiapeMSDocument> tinnyMiapeMS) {
		...
		final String mzMLfile = miapeExtractorInputParameters.getAssociatedMSFileName();

		final String mzIdentMLFile = miapeExtractorInputParameters.getInputFileName();
	...
	private void extractMIAPEfromMGFFile(MiapeXmlFile<MiapeMSDocument> miapeMsXML)
		...
		uploadedMGFFile = miapeExtractorInputParameters.getAssociatedMSFileName();

		firePropertyChange(NOTIFICATION, null, "Extracting data from MGF file + metadata...");
		taskStatus = CONVERTING;

		final MiapeXmlFile<MiapeMSDocument> miapeMSWithResultingData = addResultingData(miapeMsXML,
	...
	private void extractMIAPEfromMzIdentMLFile(MiapeXmlFile<MiapeMSDocument> miapeMsXML) throws IOException {
		...
		if (miapeMsXML != null) {
			...
			int idMS = -1;
			final Integer associatedMiapeMSID = miapeExtractorInputParameters.getAssociatedMiapeMS();
	...
	private MiapeXmlFile<MiapeMSDocument> addResultingData(MiapeXmlFile<MiapeMSDocument> miapeMsXML,
		...
		final ResultingData resultingData = getResultingDataFromUploadedFile(projectName);
		if (resultingData != null) {
			final ResultingDataAdapter resultingDataAdapter = new ResultingDataAdapter(resultingData,
					new ObjectFactory(), new MsControlVocabularyXmlFactory(new ObjectFactory(), getCvManager()));
			try {
				final File file = miapeMsXML.toFile();
				final JAXBContext jc = JAXBContext.newInstance("org.proteored.miapeapi.xml.ms.autogenerated");
				...
				final MiapeMSDocument doc = new MiapeMSDocumentImpl(miapeMS, null, getCvManager(), null, null);
				return doc.toXml();
	...
	private void extractMIAPEfromXTandemXMLFile(MiapeXmlFile<MiapeMSDocument> miapeMsXML) throws IOException {
		...
		final String xTandemXMLfile = miapeExtractorInputParameters.getInputFileName();
	...
	private void extractMIAPEfromDTASelectFile(MiapeXmlFile<MiapeMSDocument> miapeMsXML)
		...
		final String dtaSelectFile = miapeExtractorInputParameters.getInputFileName();
	...
	private void extractMIAPEfromPepXMLFile(MiapeXmlFile<MiapeMSDocument> miapeMsXML)
		...
		final String pepXMLFile = miapeExtractorInputParameters.getInputFileName();
	...
	private void extractMIAPEfromTSVFile(MiapeXmlFile<MiapeMSDocument> miapeMsXML,
		...
		final String tsvSelectFile = miapeExtractorInputParameters.getInputFileName();
	...
	private void extractMIAPEfromProteinPilotFile(MiapeXmlFile<MiapeMSDocument> miapeMsXML)
			throws IOException, MethodNotSupportedException {

		final String proteinPilotSelectFile = miapeExtractorInputParameters.getInputFileName();
		...
		if (miapeMsXML != null) {
			firePropertyChange(NOTIFICATION, null,
					"Extracting data from ProteinPilot peptide summary file + metadata...");
			final MiapeXmlFile<MiapeMSDocument> miapeMSWithResultingData = addResultingData(miapeMsXML,
					miapeExtractorInputParameters.getProjectName());
			// log.info(miapeMsXML);

			identifiers = miapeLocalExtractor.storeMiapeMSMSIFromProteinPilot(proteinPilotSelectFile,
					miapeMSWithResultingData.toBytes(), miapeExtractorInputParameters.getProjectName());

		} else {
			firePropertyChange(NOTIFICATION, null, "Extracting data from ProteinPilot peptide summary file...");
			...
			identifiers = miapeLocalExtractor.storeMiapeMSIFromProteinPilot(proteinPilotSelectFile, idMS,
					miapeExtractorInputParameters.getProjectName());
	...
	private void extractMIAPEfromPrideFile(String miapeType) {
		...
		final String prideFile = miapeExtractorInputParameters.getInputFileName();
		...
		String miapeMSMetadataFilePath = null;
		if (miapeMSMetadata != null) {
			final MiapeXmlFile<MiapeMSDocument> miapeMsXML = miapeMSMetadata.toXml();
			miapeMSMetadataFilePath = miapeMsXML.toFile().getAbsolutePath();
			// just upload metadata file if not localprocessing
			// just add resulting data if not localprocessing
	...
	private String getFirstInputFileName() {
		if (miapeExtractorInputParameters.isMzMLSelected())
			return miapeExtractorInputParameters.getAssociatedMSFileName();
		if (miapeExtractorInputParameters.isMzIdentMLPlusMGFSelected())
			return miapeExtractorInputParameters.getInputFileName();
	...
	public void done() {
		...
		try {
			...
			if (id_msi != null || id_ms != null) {
				...
				if (msUrl != null || msiUrl != null) {
					result = new MiapeExtractionResult(identifier);
					if (msUrl != null)
				...
				result = new MiapeExtractionResult(identifier, errorMessage);
				firePropertyChange(MIAPE_CREATION_ERROR, null, result);
			}
	...
	public void setTaskProgress(int aValue) {
		setProgress(aValue);
	}

	public MiapeExtractionResult getResult() {
		return result;
	}

	public boolean isGeneratingMS() {
		return getParameters().getAssociatedMSFileType() != null;
	}

	public void checkConsistency() throws MiapeDataInconsistencyException {
		// log.info("Checking consistency of task " + getRunIdentifier());
		final MiapeExtractionRunParameters parameters = getParameters();
		final InputFileType inputFileType = parameters.getInputFileType();
		if (inputFileType == null) {
			throw new MiapeDataInconsistencyException("File type is missing");
		}
		if (parameters.getInputFile() == null) {
			throw new MiapeDataInconsistencyException("Input file is missing");
		}
		if (!parameters.getInputFile().exists()) {
			throw new MiapeDataInconsistencyException("Input file '" + parameters.getInputFile().getAbsolutePath()
					+ "' doesn't exist or is not longer available");
		}
		if (inputFileType == InputFileType.TABLETEXT) {
			if (parameters.getSeparator() == null) {
				throw new MiapeDataInconsistencyException(
						"A text separator has to be selected when using table text input files");
			}
		} else {
			if (parameters.getSeparator() != null) {
				throw new MiapeDataInconsistencyException(
						"A text separator cannot be used with '" + inputFileType.getPrimaryFileDescription() + "'");
			}
		}
		if (parameters.getAssociatedMSFileType() != null) {
			if (parameters.getAssociatedMSFile() == null) {
				throw new MiapeDataInconsistencyException(
						"Format of associated MS file is selected but the file is missing");
			} else if (!parameters.getAssociatedMSFile().exists()) {
				throw new MiapeDataInconsistencyException(
						"You selected a format of an associated MS file but the file is missing");
			}
		}
		if (parameters.getAssociatedMSFile() != null) {
			if (parameters.getAssociatedMSFileType() == null) {
				throw new MiapeDataInconsistencyException("Associated MS file type is missing");
			}
		}
	}
